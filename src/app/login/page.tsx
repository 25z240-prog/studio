      ...
      if (error.code) {
        switch (error.code) {
          ...
          case 'auth/user-not-found':
            // This case is less likely now since we check first, but good to have
          ...
          case 'auth/email-already-in-use':
            // Should not happen with new logic, but as a safeguard
            description = "This email is already in use. Please try logging in normally.";
            break;
          default:
             if (!error.message.includes('permission-denied')){ // Avoid showing generic firebase error for permission issues
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-transparent p-4">
      ...
      <Card className="w-full max-w-sm">
        <CardHeader className="text-center">
          ...
          <CardDescription>
            Enter your credentials. A new account will be created if you don't have one.
          </CardDescription>
        </CardHeader>
        <form onSubmit={handleLogin}>
          ...
          <CardFooter>
            <Button className="w-full" type="submit" disabled={isSubmitting}>
              {isSubmitting ? 'Authenticating...' : 'Sign In or Sign Up'}
            </Button>
          </CardFooter>
        </form>
      </Card>
